...
public class SceneOptions : MonoBehaviour
{
    ...
    public void PlayMusic()
    {
        ...
        switch (SceneManager.GetActiveScene().name)
        {
            ...
            case "MenuScene":
                clip = AudioManager.LoadClipFromResources("MenuMusic");
                break;
            // load the game music from the resources folder using AudioManager
            case "GameScene":
                clip = AudioManager.LoadClipFromResources("GameMusic");
                break;
        }
        ...
        if (clip)
        {
            AudioManager.PlayBGM(clip, MusicTransition.LinearFade);
        }
    }
    ...
    {
        if (flag)
            AudioManager.PlayOneShotFromResources("Button", LoadGame);
        else
            AudioManager.PlayOneShotFromResources("Button", LoadMainMenu);
    }

    void LoadGame()
    {
        // fade out current music and fade in next music in 1s
        AudioManager.PlayBGMFromResources("GameMusic", MusicTransition.LinearFade);
    ...
    {
        // fade out current music and fade in next music in 1s
        AudioManager.PlayBGMFromResources("MenuMusic", MusicTransition.LinearFade);
    ...
    {
        if (flag)
            AudioManager.PlayOneShotFromResources("Button", panelOptions.ShowOptionsMenu);
        else
            AudioManager.PlayOneShotFromResources("Button", panelOptions.HideOptionsMenu);
    }

    public void QuitGame()
    {
        AudioManager.PlayOneShotFromResources("Button", Exit);
    }
